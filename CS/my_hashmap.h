...
hashmap_t *hashmap_create(unsigned int _buckets) {
    ...
    map->hashFunction = betterHash;  // Changed to use betterHash
...
void hashmap_removeKey(hashmap_t *_hashmap, char *key) {
    ...
    unsigned int bucket = _hashmap->hashFunction(key, _hashmap->buckets);
    node_t **indirect = &_hashmap->arrayOfLists[bucket];
    while (*indirect != NULL) {
...
// A better hash function using the djb2 algorithm
int betterHash(char *key, int numberOfBuckets) {
